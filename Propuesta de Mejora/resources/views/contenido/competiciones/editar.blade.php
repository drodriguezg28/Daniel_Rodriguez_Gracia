...
<div class="container mt-4 bg-white p-4 rounded">
    <h2 class="h4 mb-4 text-dark">Editar Competición: {{ $competicion->Nombre }}</h2>

    <form action="{{ route('competiciones.actualizar', ['id' => $competicion->ID_Competicion]) }}" method="POST">
        @csrf
        @method('PUT')
        ...
        <div class="row g-3">
            ...
            <div class="col-md-6">
                ...
                <input type="text" name="Nombre"
                       value="{{ old('Nombre', $competicion->Nombre) }}"
            ...
            <div class="col-md-6">
                ...
                <select name="Tipo" class="form-select" required>
                    ...
                    @foreach($tipos as $tipo)
                        <option value="{{ $tipo }}"
                            {{ old('Tipo', $competicion->Tipo) === $tipo ? 'selected' : '' }}>
                            {{ $tipo }}
                        </option>
                    @endforeach
                </select>
                ...
                <select name="Pais" class="form-select">
                    ...
                    @foreach($paises as $pais)
                        <option value="{{ $pais->ID_Pais }}"
                            {{ old('Pais', $competicion->Pais) == $pais->ID_Pais ? 'selected' : '' }}>
